...
def main() -> int:
    ...
    r2 = run(
        [sys.executable, str(ROOT / "tools" / "analyze" / "build_context_anchor.py"), "-w", str(out)]
    )
    if r2.returncode != 0:
    ...
    r2b = run(
        [
            sys.executable,
            str(ROOT / "tools" / "analyze" / "build_claim_mermaid.py"),
            "--claim-tree",
    ...
    note = _REPO / "tmp" / "test_patent_note.md"
    ...
    r3 = run(
        [
            sys.executable,
            str(ROOT / "tools" / "analyze" / "lint_patent_note.py"),
            "--note",
            str(note),
            "--manifest",
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        r_setup = run(
            [sys.executable, str(ROOT / "tools" / "vault" / "setup_obsidian_vault.py")],
            env=env,
        )
        ...
        r4 = run(
            [
                sys.executable,
                str(ROOT / "tools" / "vault" / "write_patent_obsidian_note.py"),
                "--content-file",
                str(note),
                "--manifest",
                str(out / "source_manifest.json"),
        ...
        r_clues = run(
            [
                sys.executable,
                str(ROOT / "tools" / "analyze" / "validate_public_clues.py"),
                "-i",
                str(out / "public_clues.json"),
            ]
        )
